...
async function importBundledDefaultFlightModel(){
  const loader=window.__GZ_LOAD_DEFAULT_FLIGHT_MODEL__;
  if(typeof loader!=='function'){
    toast('飞控 Demo 模型加载入口未就绪','w');
    return {ok:false,errors:['Default flight model loader is not ready.']};
  }
  ...
  if(!result?.ok){
    toast((result?.errors||['飞控 Demo 模型导入失败']).join('；'),'w');
  }
  return result;
}

function downloadFlightModelPackage(pkg){
...
function clearModelTransientStateForBlankWorkspace(){
  ...
  S.faultLibrary=[];
  S.faults=[];
  S.diagnosticModel=null;
  S.diagnosticTestPoints=[];
  S.testPoints=[];
  S.installedDiagnosticTestPointIds=[];
  S.selectedDiagnosticTestPointId='';
  S.diagnosticTestPointModelSignature='';
  S.diagnosticTestPointManualCleared=false;
  S.diagnosticScanResults=[];
  S.lastDiagnosticTestPointResult=null;
  S.testPointDiagnosis=null;
  S.diagnosticLastScanAt=0;
  S.confirmedDiagnosticFaults={};
  S.confirmedDiagnosticFaultIds=[];
  delete S.compactDiagnosticFixedPoints;
  delete S.compactDiagnosticFixedPointSignature;
  delete S.compactDiagnosticPointSignature;
  try{
    window.localStorage?.removeItem(WORKBENCH_STORAGE_KEY);
  }catch(error){
    console.warn('Failed to clear saved workbench snapshot',error);
  }
}
...
function applyBlankWorkspaceState(toastMessage=''){
  ...
  clearModelTransientStateForBlankWorkspace();
...
doImportSys = async function(){
  const btn=document.getElementById('btn-imp-sys');
  const resetButton=()=>{
    if(!btn){return;}
    btn.textContent='导入系统模型';
    btn.disabled=false;
  };
  if(btn){
    btn.textContent='导入中...';
  ...
  try{
    return await importBundledDefaultFlightModel();
  }catch(error){
    console.warn('Failed to import bundled flight model package',error);
    toast('飞控 Demo 模型导入失败，请稍后重试','w');
    return {ok:false,errors:[error instanceof Error?error.message:'Failed to import bundled flight model package.']};
  }finally{
    resetButton();
  }
};
window.doImportSys=doImportSys;
